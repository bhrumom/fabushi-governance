...
| Requirement | Implementation | State | Required evidence |
|---|---|---|---|
| `M9-PAY-002-CATALOG-API-001-R01` developer/app authorized batch create/update | Rust control-plane batch route + Platform proxy + Electron/BotFather bridge | IMPLEMENTED / CI PENDING | exact-head Rust/Platform/Electron contracts |
| `M9-PAY-002-CATALOG-API-001-R02` price revision and historical fact preservation | price revision writes + `0021` forward-only adoption migration | IMPLEMENTED / CI PENDING | migration/contract evidence, no product/order recreation |
...
| `M9-PAY-002-CATALOG-API-001-R04` official path parity and Alipay APP pay | Global Dharma uses same developer API; Alipay remains `alipay.trade.app.pay` | IMPLEMENTED / CI PENDING | source/contracts + packaged journey |
| `M9-PAY-002-CATALOG-API-001-R05` protected delivery | PR -> required CI -> protected merge -> canonical readback -> package/E2E evidence | PENDING | PR, Actions, merge SHA, screenshots/video/trace/report/logs |
