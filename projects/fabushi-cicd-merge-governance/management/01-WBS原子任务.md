...
| FCM-010.13.11 | Close installed App-owned 27-category full journey after semantic/list-state repairs | yes | exact run-owned device; <=100 finds and generation-sensitive actions remain fail-closed; all 27 categories PASS; READY -> finish -> fresh snapshot -> exact logout; controller + target + truthful gate success; complete video/trace/log/screenshots/artifact lineage | in-progress — PR #2558 exact head `9aee5e73...` received traceable REVIEW-PASS before protected merge; canonical `b8fb73466899...` produced immutable Release `desktop-1.2.56-b8fb73466899`, fresh controller `34696372043` and fresh target `34696426684/1`. The run proved the repaired `unread-none` baseline, then failed deterministically because production `KernelConversationProvider` kept Mahayana assistant `unread_count` at 0, so exact `unread-positive` never appeared. Current repair branch `fix/fcm-010-13-11-runtime-unread-transition` received independent `REVIEW-FAIL/BLOCKED` on exact head `2e65a0a6...` (`P1=1`, `P2=1`): global read state crossed shared-provider conversations and the regression stopped at Electron mocks. Repair commit `25f95c1f...` scopes read state per conversation; a real production-adapter Rust regression is being added across FeatureHost -> Runtime clamp -> Kernel/provider. The PR remains draft and still requires new exact-head CI, independent re-review, protected merge and a brand-new production target. |
