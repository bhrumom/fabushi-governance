...
## 2026-09-09 / GBF-806 ChatGPT DMG Bot 单会话体验

- Scope: 依据本机 ChatGPT.dmg 的 Electron/Chromium 可观察行为，将 Bot 单会话交互 clean-room 重写到当前 Electron、原生 iOS、原生 Android 与 Mahayana 架构；历史 Flutter 客户端明确排除。
- Implemented: Desktop Bot transcript document flow、Markdown/code、streaming cursor、thinking/action rows、copy/edit/regenerate、停止、排队、草稿、滚动位置保护；Android/iOS 按 Bot 保留 transcript/draft 并展示 streaming 状态。
- Provenance: 未复制 ChatGPT.app 的 app.asar、私有 renderer、品牌资源或二进制；开源参考仅用于公开行为/架构调查，未新增第三方依赖。
- Verification: 隔离工作副本中的 Desktop renderer typecheck/build 与 Android compileDebugKotlin 已通过；iOS 工程生成、GitHub CI、受保护 main、三端 packaged/simulated-user E2E、视觉证据和 exact-SHA Release 尚未完成。
- Status: GBF-806 = IMPLEMENTED；GBR-019 = IMPLEMENTED（门禁未闭合），项目继续保持 active。
- Next: 触发 Electron/native-mobile workflows，补 iOS 编译与跨 Bot transcript/draft、streaming、interrupt/queue、scroll 视觉/交互证据；通过后再推进 TESTED/E2E_VERIFIED。

## 2026-09-09 GBF-806 continuation — CLI steps and interactive results

Added contiguous operation-scoped collapsible step groups, interrupted state, late-terminal event protection, request-owner capture, active-transcript hydration protection and Mini App read-race protection. Installed Mini App replies preserve a clickable result pointer across reload using existing message payload persistence and capability-checked opening. Complete HTML fences can be previewed only on explicit click in an opaque-origin sandbox with no host bridge/network/form permissions. Updated existing packaged Mini App journey to open the result card.

Lightweight in-memory component contracts pass (grouping, operation separation, stopped state, complete/incomplete HTML, inert Markdown, result card). No local app build or native/E2E execution performed in this continuation. CI and packaged acceptance remain pending; status IN_PROGRESS. New research and recovery evidence recorded in the task.
