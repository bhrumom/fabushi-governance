...
| GBF-410 | M4 | ChatGPT 同账号 MCP 实时控制 GitHub Actions Runner | GBF-409 | Streamable HTTP OAuth MCP；account-scoped Runner；包内 runtime live test；trace/evidence | OAuth/gateway/agent/package/live Runner E2E | evidence/GBF-410 | IN_PROGRESS | remote deploy/PR/CI/live ChatGPT journey pending | 完成 endpoint 部署并运行真实交互式 workflow |
| GBF-411 | M4 | 统一 Web MCP / App MCP / 原生语义 Agent Surface | GBF-409,410 | `fabushi.app.*` 跨 Web/Electron/iOS/Android 同合同；远程设备动态发布；第三方 App 语义控制保留 | shared schema + bridge security + package handshake + Electron/Android/iOS E2E | evidence/GBF-411 | IN_PROGRESS | PR/CI/protected-main/package/live Runner pending | 完成 GitHub CI、修复平台编译/E2E、合并后运行 exact-main live MCP 验收 |
...
| GBF-505 | M5 | 移除生产 Grok 视觉/runtime 依赖 | GBF-501..504 | production dependency audit clean | dependency/source audit + protected-main packaged E2E | evidence/GBF-505 | IN_PROGRESS | PR #2202 CI/main/Release closure pending | merge #2202 after required checks, then canonical-main E2E/Release |
...
| GBF-509 | M5 | Grok study 头像动画行为 parity | GBF-501..504 | Fabushi-owned spring/state-eye/micro-motion/overlay parity；禁止复制 proprietary geometry/assets | provenance guard + exact-head CI + packaged Electron E2E + Release lineage | evidence/GBF-509 | RELEASED | none | GBF-805 final parity closure |
