...
- Status: `IN_PROGRESS / INSTALL_URL_FIX_PENDING`.
...
- Raw source readback before the fix: the commit-pinned URL returned HTTP 200, size `234862`
  and SHA-256 `30ec1f70e0c14a8ebbd530b2cf0186a2d63690bf85e45b8ec8d0e1a81090e9e7`.
- Current follow-up state: code is patched locally for the raw commit URL; production
  deployment and live contract readback are pending. No local build or test was run.
- Next: merge the URL correction through protected `main`, deploy the Worker, and verify
  all three catalog URLs contain the declared commit before asking the user to retry.
