...
| MSR-403 | Standalone CLI one-line install + same-account live device auto-registration + ephemeral runner lifecycle | MSR-103,401 | installed/logged-in CLI appears in official MCP; ephemeral runner disappears offline; release installers are standalone | Actions compile/package + exact artifact install + official MCP discovery/control | passed | closed with package run `35186579174`, site run `35194816512`, final three-platform MCP run `35195155342`, and empty post-finish `list_devices` |
